...
/**
 * @param {String} _service_json
 * @param {Function} _callback
 * @returns {Real}
 */
function bluetooth_le_server_add_service(_service_json, _callback)
{
    var __available__ = __GMBluetooth_is_available();
    if (!__available__) return;
    ...
    // param: _service_json, type: String
    if (!is_string(_service_json)) show_error($"{_GMFUNCTION_} :: _service_json expected string", true);
    buffer_write(__args_buffer__, buffer_u32, string_byte_length(_service_json));
    buffer_write(__args_buffer__, buffer_string, _service_json);
